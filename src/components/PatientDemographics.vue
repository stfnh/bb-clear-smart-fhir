  ...
  <div class="container">
    ...
    <h2 class="subtitle">
      ...
      <span v-if="patient.birthDate"
        ><strong> {{ age }}</strong> year old</span
      >
      <strong v-if="patient.maritalStatus">
        {{ patient.maritalStatus.text | lowerCase }}
      </strong>
      <strong v-if="patient.gender"> {{ patient.gender }}</strong>
      <span v-if="languages">
        who speaks <strong>{{ languages }}</strong></span
      >.
    </h2>
  ...
  computed: {
    ...
    languages() {
      if (this.patient.communication) {
        return this.patient.communication
          .map((c) => c.language.text)
          .join(', ');
      }
      return null;
    }
